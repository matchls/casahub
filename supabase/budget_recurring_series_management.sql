...
-- recurring_occurrences and delete_recurring_budget_occurrence, before any
-- write — see that function's comment for why this serialization is
-- necessary and how it resolves concurrently.
--
...
-- entry_month >= v_entry_month bound. recurring_expense_id and entry_month
-- are never written by this function, so guard_recurring_budget_entry_
-- update's invariants continue to hold untouched.
-- =============================================================================
...
BEGIN
  ...
  SELECT end_month INTO v_end_month
  FROM   budget_recurring_expenses
  WHERE  id = v_recurring_id;

  v_target_day := extract(day FROM target_entry_date)::integer;
  ...
  SET category_id  = target_category_id,
      ...
      kind         = target_kind,
      note         = target_note,
      entry_date   = budget_recurring_occurrence_date(entry_month, v_target_day)
  WHERE recurring_expense_id = v_recurring_id
    AND entry_month >= v_entry_month
    AND (v_end_month IS NULL OR entry_month < v_end_month);
END;
$$;
